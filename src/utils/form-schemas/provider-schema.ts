...
export const ProviderSchema = yup.object().shape({
  ...
  contactName: yup.string().min(2, 'Contact too short').max(50, 'Contact too long'),
  mobileNumber: yup.string().min(2, 'Mobile number too short').max(15, 'Mobile number too long'),
  phoneNumber: yup.string().min(2, 'Phone number too short').max(15, 'Phone number too long'),
  fax: yup.string().min(2, 'Fax too short').max(15, 'Fax too long'),
