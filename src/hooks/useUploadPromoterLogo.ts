...
type Params = { uploadUrl: string; logoFile: File };

export const useUploadPromoterLogo = (options?: MutationOptions) => {
  const apiClient = useApiClient();

  const mutationFn: MutateFunction<string, unknown, Params> = async ({ uploadUrl, logoFile }) => {
